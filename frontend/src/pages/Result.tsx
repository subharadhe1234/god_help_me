  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen flex flex-col gap-6">
      ...
      <div className="bg-white p-4 rounded-xl shadow-md text-gray-800 text-center font-medium">
        <strong className="underline underline-offset-2">General Instructions </strong>
      ...
      <div className="grid grid-cols-1 gap-6">
        {data.medicines.map((medicine: any, index: number) => (
          <div key={index} className="bg-white p-6 rounded-xl shadow-lg">
            <h2 className="text-xl font-bold text-green-700">{medicine.name}</h2>
            <p className="text-gray-700"><strong>Dosage:</strong> {medicine.dosage}</p>
            <p className="text-gray-700"><strong>Frequency:</strong> {medicine.frequency}</p>
            <p className="text-gray-700"><strong>Route:</strong> {medicine.route}</p>
            <p className="text-gray-700"><strong>Instructions:</strong> {medicine.special_instructions}</p>
                  ...
                  <ul className="space-y-3">
                    {medicine.websites.map((website: any, idx: number) => (
                      <li key={idx} className="flex items-center justify-between border-b pb-2">
                        <img
                          src={website.image || "https://via.placeholder.com/50"}
                          alt={website.title || "No Image"}
                          className="w-12 h-12 rounded-md object-cover"
                        />
                        <div className="flex-1 ml-4">
                          <p className="text-gray-900 font-semibold">{website.title || "Unknown Product"}</p>
                          <p className="text-green-700 font-bold">Rs.{website.price}</p>
                        </div>
                        <a
                          href={website.link}
                          target="_blank"
