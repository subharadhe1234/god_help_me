...
class UserHistory(Base):
    ...
    __table_args__ = (UniqueConstraint('jwt_token', 'transaction_id', name='unique_transaction_per_token'),)
...
def store_data(session, jwt_token, transaction_id, site_content, image_path=None):
    ...
    new_entry = UserHistory(jwt_token=jwt_token, transaction_id=transaction_id, site_content=json.dumps(site_content), image=image_data)
    try:
...
def get_all_history(session, jwt_token):
    return session.query(UserHistory).filter_by(jwt_token=jwt_token).all()
